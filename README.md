...
### 2. 配置 163 邮箱和 5 个 Secrets

先完成 163 邮箱的 SMTP 设置，这是必须步骤：
...
去 163 邮箱后台立即停用或重新生成授权码，然后更新仓库里的 `SMTP_PASS` secret。
...
- 默认期刊先给一个合理样例
- 默认关键词先给一个合理样例
- README 首页只保留 `Fork -> 配 163 邮箱 -> 配 Secrets -> Run workflow`
